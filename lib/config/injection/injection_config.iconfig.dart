...
void $initGetIt(GetIt g, {String environment}) {
  g.registerFactory<AuthRepository>(() => AuthRepository(g<Api>()));
  g.registerFactory<AuthStore>(() => AuthStore(g<AuthRepository>()));
  g.registerLazySingleton<GlobalConfig>(() => GlobalConfig());
  g.registerFactory<HomeRepository>(() => HomeRepository(g<Api>()));
  g.registerFactory<HomeStore>(() => HomeStore(g<HomeRepository>()));
  g.registerLazySingleton<UserRepository>(() => UserRepository(g<Api>()));

  //Register prod Dependencies --------
  if (environment == 'prod') {
    g.registerLazySingleton<Api>(() => ApiProd());
  }

  //Register dev Dependencies --------
  if (environment == 'dev') {
    g.registerLazySingleton<Api>(() => ApiDev());
  }
}
